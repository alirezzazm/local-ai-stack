...
## جستجو و یادگیری از اینترنت (RAG زنده)
دکمه‌ی **🌐 یادگیری** در بالای UI یک پنل باز می‌کند: یک موضوع می‌دهی → سرور در اینترنت جستجو می‌کند (DuckDuckGo، بدون کلید API)، چند صفحه را می‌خواند، با DAZ خلاصه‌ی فارسی می‌سازد و در `knowledge/` ذخیره می‌کند. از آن به بعد، وقتی سؤالی مرتبط بپرسی، آن دانش خودکار به مدل داده می‌شود (RAG).
